...
**Your Answers here:**
1. Looking at the Principal variance component table generated above, 6 factors allow us to explain 52.29% of the variance. 

2. Component 1 and component 2 make up for the highest weights in absolute value, at 21.94% explained variance and 12.25%, respectively. 

3. The first 10 components can be interpreted as "principal components". Their eigenvalue is above 1, they explain over 50% of the variance and they sit on the scree plot line elbow.

4. a) Pre-crisis bull market years: November 1, 2002 to October 1, 2007 
```{r echo=TRUE, eval=TRUE, comment=NA, warning=FALSE,error=FALSE, message=FALSE, prompt=FALSE, tidy=TRUE}
...
b) Financial crisis market years: October 1st 2007 to March 1st 2009
```{r echo=TRUE, eval=TRUE, comment=NA, warning=FALSE,error=FALSE, message=FALSE, prompt=FALSE, tidy=TRUE}
...
5. It appears that returns in bull years are less driven by 1-2 components, compared to bear years. In bull years, 7 components explain 50% of the variance, whereas in bear years, 2 components explain over 50% of the variance. ?ma
...
**Your Answers here:**
1. It looks at moving averages of different "window length" and how to weight compared to each other, in order to sum up to 1. 
2. 
```{r echo=TRUE, eval=TRUE, comment=NA, warning=FALSE,error=FALSE, message=FALSE, prompt=FALSE, tidy=TRUE}
signal_used = 0*futures_data 
MAfreq<-seq(10,500,by=20)
for (iter in 1:length(MAfreq))
  signal_used = signal_used + sign(apply(futures_data,2, function(r) ma(r,MAfreq[iter])))
signal_used = t(apply(signal_used,1,function(r) {
  res = r  
...
colnames(signal_used) <- colnames(futures_data)
strategy_by_future <- scrub(shift(signal_used,5)*futures_data) 
trading_strategy = apply(strategy_by_future,1,sum)
names(trading_strategy) <- rownames(futures_data)
```
