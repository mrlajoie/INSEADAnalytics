...
##### Answers:
1. Some questions are more correlated with one another than others. The highest correlation are between Q27 and Q28, at about 0.6 of correlation. Both relate to boating being a #1 activity or a passion. There seems to be also slightly higher correlation between questions touching on ego issues (sence of achievement, status, etc.)
2. These correlations imply that customers are more likely to answer clusters of certain type of questions if they have similar attributes. 
...
**Your Answers here:**
1. Both the table and the plot show how much each of the component contribute to the overall variance within the model. The plot allows us to visually see how much each of the components, in decreasing variance contribution, add to the overall model.
2. The plot has this shape as the component are in decreasing order of importance, the largest contributor to the variance arriving first, followed by the incremental decreasing contributions of the other factors. The sum of the variance is = to 100% and as such, the next component should always be less than the preceding component, making it impossible for the line to be increasing.
3. Ideally, we would see a small number of components contributing a large amount of variance, i.e. the stick part of the curve should be longer. 
...
**Your Answers here:**
1. The first couple of factors correspond to those component that explain most of the variance. In this case, C1 corresponds to those that boost the ego, C2, those that see boating as a hobby, C3 those that like brands, C4 the repairmen, and C5 those that aim for low price/simplicity. The five of them make good business sense.
2. I would choose those factors that explain most of the variance, while not being correlated and that make sense both in the intepretation and business sense. 
3. I would see the factors I choose as those most likely to explain the behaviour of that customer segment.
4. The biggest lesson here is that one has to statistical identify which data is relevant while intepreting it in the larger business context. 
...
**Your Answers here:**
1. There appears to be 3-5 segments in our market, depending on how much we want to segment them. 
2. The 3 segments here correspond to 1- the hobby person 2-the expert/repairmen 3- the status seeker. 
3. If we change the number of factors, we may find different well performing analysis. 
4. I would suggest they value each of these segments - i.e. how much do each of these clusters contribute to their revenues and overall bottomline. Based on that information, I would look into 1-redesigning the products to attract that segments 2-reevaluate our sales strategy to the segment 3- re-assess the overall strategy in the company in the upcoming years (i.e. do they want to appeal to those segments or move elsewhere?)
